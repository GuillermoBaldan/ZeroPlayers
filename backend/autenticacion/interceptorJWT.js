...
exports.interceptorJWT = function (request, response, next) {
  ...
  if (
    (request.method.toUpperCase() == "POST" && urlSinParametros == "/login") ||
  ...
  if (!authorization) {
    response
      .status(401)
      .json({ codigo: 401, mensaje: "Falta la cabecera de autorización" });
    return;
  }
  ...
  if (trozos.length != 2 || trozos[0].toLowerCase() != "bearer") {
    response.status(400).json({
      codigo: 401,
      mensaje: "La cabecera authorization está mal construida",
    });
    return;
  }

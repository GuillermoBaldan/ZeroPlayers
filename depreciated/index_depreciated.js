...
function Init() {
  ...
  matrixStage = matrixGenerationInit(
...
function matrixGenerationInit(
  ...
  completeStageAux = generateCompleteStageInit(
    staticStageAux,
    dynamicElementsArray,
    squareSide
  );
...
function matrixGeneration(
  ...
  let completeStageAux;
  let staticStageAux;
  staticStageAux = generateStaticStage(
    staticStage,
    legend,
    wideDimension,
    heightDimension
  );
  completeStageAux = generateCompleteStage(
    staticStageAux,
    dynamicElementsArray,
    squareSide
  );
...
function oneStepSimulation(
  ...
  setTimeout(function () {
    ...
    let completeStageAux;
    completeStageAux = oneSimulationStepCalculation(
      index,
      staticStage,
      dynamicElementsArray
    );
...
function oneSimulationStepCalculation(
  index,
  staticStage,
  dynamicElementsArray
) {
...
function generateCompleteStage(staticStage, dynamicElementsArray, squareSide) {
  ...
  dynamicElementsArray.forEach((item) => {
    completeStageAux[item.y + (heightDimension / squareSide - 1)][item.x] =
      item.color;
  });
  return completeStageAux;
}
...
function generateStaticStage(legend, wideDimiension, heightDimension) {
  ...
  for (b = 0; b < heightDimension; b++) {
    ...
    for (a = 0; a < wideDimiension; a++) {
      row.push(
        materialGeneration(legend)[Math.floor(Math.random() * numberMaterials)]
      );
    }
    stageAux.push(row);
  }
...
function generateCompleteStageInit(
  staticStage,
  dynamicElementsArray,
  squareSide
) {
  ...
  dynamicElementsArray.forEach((item) => {
    completeStage[-item.y + (heightDimension / squareSide - 1)][item.x] =
      item.color;
  });
